...
static void ReadRealIndex( demux_t *p_demux )
{
    ...
    i_id = VLC_FOURCC( buffer[0], buffer[1], buffer[2], buffer[3] );    // 4 characters
    i_size      = GetDWBE( &buffer[4] );                                // U32_AT
    i_version   = GetWBE( &buffer[8] );                                 // U16_AT
    ...
    i_index_count = GetDWBE( &buffer[10] );                             // U32_AT. Supplied by user data, entry point. The 10,11,12,13 bytes are the index count data.
    ...
    // Replace the original malloc with calloc
    // While malloc is more efficient than calloc, but malloc and memset isn't.
    // And calloc does this job safer and prevents overflow that is possible with malloc(i_index_count+1 * sizeof(rm_index_t))
    p_sys->p_index = calloc(i_index_count + 1; sizeof(rm_index_t));

    // The default data type for the for-loop index value is signed integer.
    // Assign unsigned integer type to the counter to prevent the exploit of casting unsigned integer to signed integer causing infinite loop.
    for(uint32_t i = 0; i < i_index_count; i++ )
    {
        // END: Fixes
