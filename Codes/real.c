...
static void ReadRealIndex( demux_t *p_demux )
{
    ...
    if( GetDWBE( &buffer[16] ) > 0 )
        msg_Dbg( p_demux, "Real Index: Does next index exist? %d ",
                        GetDWBE( &buffer[16] )  );
    // rm_index_t stores 3 uint32_t.
    p_sys->p_index = 
            (rm_index_t *)malloc( sizeof( rm_index_t ) * (i_index_count+1) );   // vuluneability, size of heap buffer. large i_index_count, small heap buffer.
    ...
    memset( p_sys->p_index, 0, sizeof(rm_index_t) * (i_index_count+1) );   // padding with 00s.

    for( i=0; i<i_index_count; i++ )        // counter for the loop, can over flow the heap buffer. i is signed. so i_index_count can use unsigned int to mess up the loop
    {
        if( stream_Read( p_demux->s, buffer, 14 ) < 14 )    // can read 14 bytes or not
            return ;
